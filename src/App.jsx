...
const ADMIN_ROUTES = ['admin-dashboard', 'admin-users', 'admin-sellers', 'admin-orders']
...
function AppShell() {
  ...
  function goAdmin(page = 'admin-dashboard') {
    setRoute(page)
    scrollTop()
  }
  ...
  return (
    <div className="min-h-screen bg-neone-bg text-white">
      <Header
        ...
        onOpenAdmin={() => goAdmin('admin-dashboard')}
      ...
      {route === 'admin-dashboard' && isAdmin && (
        <AdminDashboard onNavigate={goAdmin} onLogout={handleLogout} onBack={goHome} />
      )}
      {route === 'admin-users' && isAdmin && (
        <AdminUsers onNavigate={goAdmin} onLogout={handleLogout} onBack={goHome} />
      )}
      {route === 'admin-sellers' && isAdmin && (
        <AdminSellers onNavigate={goAdmin} onLogout={handleLogout} onBack={goHome} />
      )}
      {route === 'admin-orders' && isAdmin && (
        <AdminOrders onNavigate={goAdmin} onLogout={handleLogout} onBack={goHome} />
      )}
      {isAdminRoute && !isAuthenticated && (
        <Login onBack={goHome} onLoggedIn={() => goAdmin('admin-dashboard')} onGoRegister={goRegister} onGoForgotPassword={goForgotPassword} />
      )}
      {isAdminRoute && isAuthenticated && !isAdmin && (
        <div className="px-4 sm:px-8 py-10">
          <EmptyState
            icon={ShieldAlert}
            title="Ruxsat yo'q"
            message="Bu bo'limga faqat administratorlar kira oladi."
            ctaLabel="Bosh sahifaga qaytish"
            onCta={goHome}
          />
        </div>
      )}
